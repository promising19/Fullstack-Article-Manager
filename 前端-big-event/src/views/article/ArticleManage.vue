...
const articleList = async () => {
    ...
    // 处理数据，给数据模型扩展一个数据，分类名称，用于显示到页面上
    for (let i = 0; i < articles.value.length; i++) {
        let article = articles.value[i];
        for (let j = 0; j < categoryId.value.length; j++) {
            if (article.categoryId == categorys.value[j].id) {// 当前分类
...
// 添加文章
// import { ElMessage } from 'element-plus'
const addArticle = async(clickSatae)=>{
    // 把发布状态赋值给数据模型
    articleModel.value.state = clickSatae;
